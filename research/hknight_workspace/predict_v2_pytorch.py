...
# ----- Prediction Function -----
def predict_video(video_path, model_path, label_map, device):
    """
    Process a video file frame-by-frame to generate three streams:
    ...
    with mp_hands.Hands(min_detection_confidence=0.3,
        ...
        # Determine frame sampling interval to achieve approximately 25 FPS.
        actual_fps = cap.get(cv2.CAP_PROP_FPS)
        desired_fps = 25
        frame_interval = max(1, int(round(actual_fps / desired_fps)))
        print(f"Video actual FPS: {actual_fps}, sampling every {frame_interval} frame(s) to achieve ~{desired_fps} FPS")
        ...
        while True:
            ...
            # Use the non-annotated copy for spatial cropping (removing any drawn pose).
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Predict action from video using a three-stream model (PyTorch).")
    ...
    predict_video(args.video, args.model, label_map, device)
